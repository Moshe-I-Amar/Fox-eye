...
const useLocationTracking = ({ realtimeEnabledRef, socketInitializedRef, onFirstLocation }) => {
  ...
  useEffect(() => {
    ...
    const send = async (lat, lng) => {
      ...
      try {
        if (realtimeEnabledRef.current && socketService.isSocketConnected()) socketService.updateLocation([lng, lat]);
    ...
    const watchId = navigator.geolocation.watchPosition(async ({ coords: { latitude, longitude } }) => {
      const loc = [latitude, longitude];
      if (isValidCoords(loc)) {
        setUserLocation(loc);
        if (!hasCenteredRef.current) { onFirstLocation?.(loc); hasCenteredRef.current = true; }
      }
      await send(latitude, longitude); setLocationLoading(false);
